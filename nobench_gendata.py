...
def generate_object(seed, total_seeds, corpus, word_counts):
    ...
    #Creating a string of among 10 possible strings.
    if DEEPLY_NESTED:
        level_8 = {
            "deep_str_agg": encode_string(seed % 10),
            "deep_str_single": encode_string(seed)
        }
        level_7 = {
            "level_8": level_8
        }
...
def generate_objects(base_handler,
    ...
    for i, seed in enumerate(secondary_seeds):
        if i == len(secondary_seeds) - 1:#HOLY FUCK HE HAD IT SET TO SEEDS. CHRIST
...
def main():
    ...
    if args:
        print
        "Unrecognized option (use -h for help)"
        sys.exit(1)
...
def main_non_cli(num_objs=0, mongo=False, outfile=GENERIC_FILENAME,):
    ...
    pretty = False if mongo else True

    print >> sys.stderr, "Downloading Brown Corpus..."
    nltk.download("brown")

    outfile_name_parts = outfile.rsplit(".", 1)
    if len(outfile_name_parts) == 2:
        secondary_outfile_name = (outfile_name_parts[0] +
                                  "_extra" + "." + outfile_name_parts[1])
    else:
    ...
    main_handler = JSONFileWriter(outfile, pretty, not mongo)
    extra_handler = JSONFileWriter(secondary_outfile,
                                   pretty,
                                   not mongo)
